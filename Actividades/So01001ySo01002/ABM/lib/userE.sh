...
verifUser() #Esta funcion de encarga del correcto ingreso del nombre de un usuario perteneciente al sistema
{
	...
	do
		...
		if ! test -z $dato
			then
			if test $(grep "^$dato:x:[1-9][0-9]\{3\}:*" '/etc/passwd'|wc -l) -ne 0 #Verifica que haya algun resultado para dicho usuario en el registro de usuarios del sistema, cuya GID sea de 4 cifras 
			then
