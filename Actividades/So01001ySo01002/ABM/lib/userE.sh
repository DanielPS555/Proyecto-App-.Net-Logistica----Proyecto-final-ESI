...
verifUserTODO() #Esta funcion de encarga del correcto ingreso del nombre de un usuario perteneciente al sistema
{
	verifM=0
	while test $verifM -eq 0
	do
		...
		if ! test -z $dato
			then
			if test \( $(grep "^$dato:x:[0-9]*:*" '/etc/passwd'|wc -l) -ne 0 \) -o \( \( "$dato" == "root" \) -a \( -n "$1" \) \) #Verifica que haya algun resultado para dicho usuario en el registro de usuarios del sistema, cuya GID sea de 4 cifras 
			then
				respuesta=$dato #devuelve el dato
				verifM=1 #rompe el bucle 
			else
